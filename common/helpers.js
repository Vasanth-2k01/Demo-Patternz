...
exports.isHavePermission = (menuName, permissionType) => {
  return async (req, res, next) => {
    try {
      console.log('isHavePermission', req.user, 'req.user', menuName, 'menuName', permissionType, 'permissionType');
      if (req.user) {
        if (req.user.menus && req.user.menus.length) {
          ...
          if (RequestedMenu && RequestedMenu.permissions && RequestedMenu.permissions.length) {
            let RequestedPermission = _.find(RequestedMenu.permissions, (permission) => {
              return permission.permission_type == permissionType && permission.is_permission_allowed == 1
            })
            if (RequestedPermission && RequestedPermission.permission_id) {
              ...
              console.log(`You dont have ${permissionType} permission for ${menuName} menu`);
              throw new CustomError('You dont have permission for this menu', 401, null)
            }
          } else {
...
exports.GetAllList = (body, results, rowcount, page, limit, offsets, link) => {
  ...
  try {
    URL = process.env.NODE_ENV = "development" ? process.env.ENVIRONMENT_URL : '';
    let data = {
      data: results, current_page: page,
      path: `${URL}/${link}`,
      from: page == 1 && !results.length ? 0 : ((page - 1) * limit) + 1,
      to: ((page - 1) * limit) + results.length,
      last_page: results.length > 0 ? parseInt(Math.ceil(parseFloat(rowcount / limit))) : null,
      first_page_url: `${URL}/${link}?page=1`,
      last_page_url: results.length > 0 ? `${URL}/${link}?page=${parseInt(Math.ceil(parseFloat(rowcount / limit)))}` : null,
      next_page_url: results.length > 0 ? (offsets + limit) < rowcount ? `${URL}/${link}?page=${page + 1}` : null : null,
